...
public class CalculateBill extends JFrame implements ActionListener{
    ...
    CalculateBill(){
        ...
        try{
            Conn c = new Conn();
            ResultSet rs = c.conn().executeQuery("select * from customer");
            while(rs.next()){
        ...
        try{
            Conn c = new Conn();
            ResultSet rs  = c.conn().executeQuery("select * from customer where meter = '"+c1.getSelectedItem()+"'");
            while(rs.next()){
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == b1){
            ...
            try{
                Conn c = new Conn();
                ResultSet rs = c.conn().executeQuery("select * from tax");
                while(rs.next()){
